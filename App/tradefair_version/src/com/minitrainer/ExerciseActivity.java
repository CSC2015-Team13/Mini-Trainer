 ...
 * @author Callan White
 * Description: Activity where the user can complete the exercises
 */
public class ExerciseActivity extends Activity implements OnClickListener
{
	...
	final int NOT_VISIBLE_TEXT_AT = 5;
	int [] rewards = {0,12,9,7,9,10,11,9,8};
	int NUMOFEX = 9;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
    	...
    	lastExTime = 0;
		mpDone = MediaPlayer.create(getApplicationContext(), R.raw.exdone3);
    ...
    private void setupExercises()
    {
    	...
    	ex[0] = new Exercise(this,rewards[0],R.id.exButton0,R.id.exPanel0,R.id.arrLeft0,R.id.stepProgress0,R.id.arrRight0,R.id.currStep0,ex0Txt,R.id.exGuide0,ex0Img, R.id.complete_ex0,0);
    	ex[1] = new Exercise(this,rewards[1],R.id.exButton1,R.id.exPanel1,R.id.arrLeft1,R.id.stepProgress1,R.id.arrRight1,R.id.currStep1,ex1Txt,R.id.exGuide1,ex1Img, R.id.complete_ex1,10800000);//15
    	ex[2] = new Exercise(this,rewards[2],R.id.exButton2,R.id.exPanel2,R.id.arrLeft2,R.id.stepProgress2,R.id.arrRight2,R.id.currStep2,ex2Txt,R.id.exGuide2,ex2Img, R.id.complete_ex2,6000000);//12
    	ex[3] = new Exercise(this,rewards[3],R.id.exButton3,R.id.exPanel3,R.id.arrLeft3,R.id.stepProgress3,R.id.arrRight3,R.id.currStep3,ex3Txt,R.id.exGuide3,ex3Img, R.id.complete_ex3,5000000);//10
    	ex[4] = new Exercise(this,rewards[4],R.id.exButton4,R.id.exPanel4,R.id.arrLeft4,R.id.stepProgress4,R.id.arrRight4,R.id.currStep4,ex4Txt,R.id.exGuide4,ex4Img, R.id.complete_ex4,5400000);
    	ex[5] = new Exercise(this,rewards[5],R.id.exButton5,R.id.exPanel5,R.id.arrLeft5,R.id.stepProgress5,R.id.arrRight5,R.id.currStep5,ex5Txt,R.id.exGuide5,ex5Img, R.id.complete_ex5,8000000);
    	ex[6] = new Exercise(this,rewards[6],R.id.exButton6,R.id.exPanel6,R.id.arrLeft6,R.id.stepProgress6,R.id.arrRight6,R.id.currStep6,ex6Txt,R.id.exGuide6,ex6Img, R.id.complete_ex6,8500000);
    	ex[7] = new Exercise(this,rewards[7],R.id.exButton7,R.id.exPanel7,R.id.arrLeft7,R.id.stepProgress7,R.id.arrRight7,R.id.currStep7,ex7Txt,R.id.exGuide7,ex7Img, R.id.complete_ex7,7500000);
    	ex[8] = new Exercise(this,rewards[8],R.id.exButton8,R.id.exPanel8,R.id.arrLeft8,R.id.stepProgress8,R.id.arrRight8,R.id.currStep8,ex8Txt,R.id.exGuide8,ex8Img, R.id.complete_ex8,6500000);
    	
    	// set rewards according to level
    	setExp(currLevel);
		...
		else
		{
			if (v instanceof ImageView)
			{
				for(int i=0;i<stepBack.length;i++)
				{
					...
					{
						ex[i].stepBack();
						if (i >= NOT_VISIBLE_TEXT_AT)focusOnView(ex[i].getPnlBtn());
						break;
					}
				}
	...
	//loads exercises' progress (locked/unlocked)
    public void loadExProgress()
    {
    	...
    	for(int i = 0;i<NUMOFEX;i++)
    	{
    		...
    		ex[i].loadState(this,btnState,timestamp,d.getTime());
    	}
    }
    ...
    private void setupAlert(final int i)
    {
		...
		.setCancelable(false).setPositiveButton("Yes!",new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog,int id) {
				...
				lastExTime = c.getTimeInMillis() * 100 + c.get(Calendar.DAY_OF_MONTH);;
				try {
					if(soundState)mpDone.start();
				} catch (Exception e) {
					e.printStackTrace();
				}
				exercises_done++;
    ...
    // opens panel at certain position
		...
		for(int i=0;i < NUMOFEX;i++)
		{
			if (pos == i)
			{
				...
				else
				{
					...
					needReload = ex[i].getRemTime(this,newDate.getTime());
					if (needReload)
					{
						loadExProgress();
					}
	...
	// save methods end
    //returns the user back to parent activity i.e. TabbedActivity with the data it collected
	@Override
	public void onBackPressed() 
	{
	    ...
	    bundle.putLong("LAST_EX_DONE_AT", lastExTime);
	...
	// class that pushes the timestamps of exercises ( will be compared to cooldowns afterwards)
