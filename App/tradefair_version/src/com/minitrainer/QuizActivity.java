 ...
 * 
 * Description: Activity designed as a mini-game(quiz) where user has a multiple choice
 */
public class QuizActivity extends Activity implements OnClickListener{
	...
	// sets appropriate exp reward for questions based on level
	...
	// sets up which questions the quiz is going to ask the user depending on the level.
	private void createQuestions()
	{
		...
		{
			qdBound = 0; // pick questions 
			quBound = 3; // from index 0 to 2
		}
		else
		{
			qdBound = currLevel;
	...
	private void pickQuestion()
	{
		if (((qs.size()> 0 && qAnswered < MAX)) || (currLevel == MAX_LEVEL && qs.size()> 0))
		{
			...
			{
				question.setText("Question " + (qAnswered+1) + "/30"+ "\n" + qs.get(currQ).getQuestion());
			}
	...
	/*
	 * Sets up an alert showing either how much experience he got during the session of answering questions
	 * OR how many questions the user got wrong or right along with accuracy level as a percentage
	 */
	...
	// find the correct answer to display in case users answer a question wrong
	...
	/*
	 * sets up data for return to TabbedActivity and goes back to it
	 */
	@Override
	public void onBackPressed() 
	{
		...
		{
			if (backTaps == 1)// prevents from going back to TabbedView in case the user from accidentally/intentionally 
			{					// presses the back button if he/she gained a level while completing the quiz and have not yet finished it
