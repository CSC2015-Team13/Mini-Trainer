...
public class QuizActivity extends Activity implements OnClickListener{
	...
	private void pickQuestion()
	{
		...
		if (qs.size()> 0 && qAnswered < MAX)
		{
			...
			for(int i=0;i<choices.size();i++)
			{
				choices.get(i).setBackgroundResource(R.drawable.button_bg1);
			         ...
			         public void run() { 
						...
						else
						{
						    ...
						    handler2.postDelayed(new Runnable() { 
						         public void run() { 
						        	 choices.get(findRightAnswer(qs.get(currQ).getAnswer())).setBackgroundColor(Color.parseColor("#44ca60"));
						         } 
						    }, 1800); 
