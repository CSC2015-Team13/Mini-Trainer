...
public class Exercise {
    ...
    {
    	System.out.println(t -timestamp + " >= "+ cooldown);
    	//int passed = (int) (((t /1000 + t % 1000)/60 + ((t /1000 + t % 1000)%60) - 
    	//					(timestamp /1000 + timestamp % 1000)/60) + (timestamp /1000 + timestamp % 1000)%60);
    	//int cooldownTemp = (cooldown / 1000 + cooldown % 1000)/60 + (cooldown / 1000 + cooldown % 1000)%60;
    	if ((cooldown/60000) -((t/60000) -(timestamp /60000)) > 0)
    	{
    		Toast.makeText(a, pb.getText().toString() + "\nwill be available in " + ((cooldown/60000) -((t/60000) -(timestamp /60000))) + " minutes", Toast.LENGTH_LONG).show();
    	}
    	else
    	{
    		...
    		if (secs > 0)
    		{
    			Toast.makeText(a, pb.getText().toString() + " exercise\nis available for completion in " + secs + "s", Toast.LENGTH_SHORT).show();
    		}
    		else
    		{
    			//this.setStateToUnlocked(a);
    			//if (panelPressed())
    			//this.foldPan();
    			// do magic stuff
    			//a.loadExProgress();
