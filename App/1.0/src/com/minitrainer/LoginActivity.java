...
public class LoginActivity extends Activity implements OnClickListener,OnTabChangeListener,OnFocusChangeListener,OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		logTrouble.setOnClickListener(this);
	...
	public void onClick(View v) 
	{
		...
		switch(v.getId())
		{
			...
			case R.id.btnRegister :				
				...
				if (TextUtils.isEmpty(usernameData) || usernameData.length() < 3 || usernameData.length() > 20)
				{
					usernameFlag = false;
				}
				
				// final check to decide what to include into Toast that will display the error
				...
				{
					if (!usernameFlag && usernameData.length() > 20)
					{
						errorUname = "Username is too long";
					}
					else
					{
						errorUname = "Username is too short";
					}
	...
	// sets up recovery alert
	public void setupRecoveryAlert()
	{
			  ...
			  new DialogInterface.OnClickListener() {
			    public void onClick(DialogInterface dialog,int id) {
				    ...
				    if (!(TextUtils.isEmpty(email) || email.length() < 5))
				    {
				    	...
				    	{
				    		setupRecoveryAlert();
				    		Toast.makeText(LoginActivity.this, "Email provided is invalid", Toast.LENGTH_SHORT).show();
				    	}
				    }
				    else
				    {
				    	setupRecoveryAlert();
				    	Toast.makeText(LoginActivity.this, "Email provided is too short", Toast.LENGTH_SHORT).show();
				    }	
			    }
			  })
	...
	// cleans password field when appropriate 
	@Override
	public void onFocusChange(View v, boolean hasFocus) {
		...
		if (v.getId() == R.id.txtUsername)
		{
			...
			if (tempUsername.equals("") || tempUsername.length() == 0 )
			{
				logPassword.setText("");
			}
		}
	...
	// checks if the device is connected ( wi-fi, mobile gprs, etc)
	...
	// attempts to register user if all fields are filled
	...
	/*
	 *  class that allows user to log in if a user with such username and passwords exists in the database(on the device or on the server);
	 *  pulls data from the server that belongs to the user which is about to log in(if device is connected);
	 *  pulls data that belongs to the user which is about to log in from sharedpreferences(local data storage) ;
	 */
	class loginUserServer extends AsyncTask<String,String,String>
	{
		...
		protected void onPostExecute(String result)
		{
			...
			{	
				...
				{
					Toast.makeText(LoginActivity.this, "Logged into local account", Toast.LENGTH_SHORT).show();
				}
			...
			else
			{
				...
				Toast.makeText(LoginActivity.this, "No connection detected,\nchecking for local account..",Toast.LENGTH_SHORT).show();
				if(db.userExists(username))
				{
	...
	/*
	 *  class that allows the user to register;
	 *  user can only register when the device is online
	 */
	class registerUserServer extends AsyncTask<String,String,String>
	{
		...
		protected void onPostExecute(String result)
		{
			...
			if(complete)
			{
				...
				Toast.makeText(LoginActivity.this, "Registration unsuccessful due to server error", Toast.LENGTH_SHORT).show();
			}
			else
			{
				if (!isConnected)
	...
	/*
	 * class which attempts to pull the data belonging to the the user which is about to log in from the server
	 */
	class pullFromServer extends AsyncTask<String,String,Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			//pDialog.dismiss();
			/* not necessary , but wont delete, comment
	...
	// class that sends a http request to the server to send a password reset (needs internet connection)
	...
	// class that pulls timestamps of exercises
