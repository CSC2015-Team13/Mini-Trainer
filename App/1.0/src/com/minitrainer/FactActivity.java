 ...
 * 
 * Description: Activity where user can see the facts the unlocked
 */
public class FactActivity extends Activity implements OnClickListener{
	...
	{
		final int panOpen = currLevel / FACT_PARAM;
		...
		if (panOpen != 0) 
		{
			...
			if (panOpen <= NOT_VISIBLE_TEXT_AT)
			{
			...
			else
			{
				...
				if (quickShow)
				{
					...
					if (quickShow && currLevel == MAX_LEVEL)
					{
